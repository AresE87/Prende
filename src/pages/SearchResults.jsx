...
import { MOCK_SPACES, ZONAS, AMENITIES } from "../lib/utils";
...
export default function SearchResults() {
  ...
  // Filtrar y ordenar
  let results = MOCK_SPACES.filter((s) => {
    if (zona && s.zona !== zona) return false;
    if (personas && s.capacity < personas) return false;
    if (s.price > maxPrice) return false;
    if (selectedAmenities.length && !selectedAmenities.every((a) => s.amenities.includes(a))) return false;
    return true;
  });
  ...
  function clearFilters() {
    setZona(""); setFecha(""); setPersonas(2);
    setSelectedAmenities([]); setMaxPrice(5000);
  }
  ...
  const activeFilters = [
    ...
    ...selectedAmenities.map((id) => {
      ...
      return { label: `${a?.icon} ${a?.label}`, onRemove: () => toggleAmenity(id) };
    }),
  ].filter(Boolean);
    ...
    <div className="min-h-screen bg-[#FAF7F2]">
        ...
        {activeFilters.length > 0 && (
          <div className="max-w-6xl mx-auto px-4 pb-3 flex flex-wrap gap-2">
            {activeFilters.map((f, i) => (
              <span key={i} className="flex items-center gap-1.5 bg-[#1C1917] text-[#F5F0E8] text-xs font-medium px-2.5 py-1 rounded-full font-['Inter']">
                {f.label}
                <button onClick={f.onRemove} className="hover:text-[#D4541B] transition-colors"><X size={12} /></button>
              </span>
            ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <p className="text-xs font-bold text-[#1C1917]/40 uppercase tracking-wider mb-3 font-['Inter']">
                    Precio máximo/hora: <span className="text-[#D4541B]">${maxPrice.toLocaleString()}</span>
                  </p>
                  <input
                  ...
                  <div className="flex flex-wrap gap-2">
                    {AMENITIES.map((a) => (
                      <button
                        ...
                        className={`text-xs px-3 py-1.5 rounded-full border transition-all font-['Inter'] ${selectedAmenities.includes(a.id) ? "bg-[#D4541B] text-white border-[#D4541B]" : "bg-white text-[#1C1917] border-[#1C1917]/20 hover:border-[#D4541B]"}`}
                      >
                        {a.icon} {a.label}
                      </button>
                    ))}
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        <p className="text-sm text-[#1C1917]/50 mb-5 font-['Inter']">
          <span className="font-bold text-[#1C1917]">{results.length}</span> espacios{zona && ` en ${zona}`}
        </p>
        ...
        {results.length === 0 ? (
          <EmptyState
            ...
            description="Probá con otros filtros o buscá en toda Montevideo."
